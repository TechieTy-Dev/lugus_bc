...
const styles = {
  wrapper:
    "w-[40%] flex flex-col border-[#d6d7d9] border-2 rounded-lg shadow-lg",
  top: "flex w-full h-[80px] bg-[#f0f1f3] p-[20px] pr-[80px] gap-[80px]",
  topHeaderText: "text lg text-left flex items-center",
  content: "flex flex-col w-full h-[400px] gap-[20px] flex-1",
  ...
  buyAgainBtn:
    "bg-[#ffd713] font-bold rounded-full p-[10px] h-[40px] w-[200px] cursor-pointer text-[#3a2802] text-center mb-[5px] mt-[10px]",
  etherscanBtn:
    "font-bold rounded-full h-[40px] w-[150px] cursor-pointer text-[#3a2802] text-center border-2 border-[#ffd713] flex justify-center items-center",
};

const Transaction = ({ item }) => {
  const { username } = useContext(LugusContext);
